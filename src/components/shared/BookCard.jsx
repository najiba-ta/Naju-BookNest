    ...
    return (
       <div>
         <div className="grid grid-cols-4 gap-6 container mx-auto my-7">
            {
               books.slice(0,4).map((book) => {
                    return <div className="mt-10 card shadow-md hover:shadow-2xl hover:-translate-y-1 transition duration-300 p-2" key={book.id}>
                        <Image className="border rounded-lg" src={book.image_url} height={800} width={400} alt={book.title}/>
        ...
        <div className="text-center mt-8 mb-5"> 
               <Link href={'/allbook'}> <button className="btn btn-soft text-xl bg-cyan-700 text-white py-6 px-6">View Details <FaArrowUpRightFromSquare /></button></Link>
            </div>
       </div>
    );
};
